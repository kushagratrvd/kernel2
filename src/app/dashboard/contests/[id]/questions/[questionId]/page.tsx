...
export default function EditQuestionPage({
  ...
  // Load contest & question data
  const { data: contestData, isLoading, error } = useQuery(
    trpc.contest.getForEdit.queryOptions({ id: contestId }),
  );

  const updateQuestionMutation = useMutation(trpc.contest.updateQuestion.mutationOptions());
  ...
  const targetQuestion = contestData?.questions?.find((q) => q.id === questionId);

  // Initialize form state
  useEffect(() => {
    if (!targetQuestion) return;

    setQuestionText(targetQuestion.questionText);
    setQuestionScore(targetQuestion.questionScore);
    setQuestionType(targetQuestion.questionType as any);

    if (targetQuestion.questionType === "mcq") {
      const opts = (targetQuestion.options as string[]) || ["Option A", "Option B"];
      setOptions(opts);
      setCorrectOption(targetQuestion.correctOption || opts[0]);
    } else if (targetQuestion.questionType === "text") {
      setTextCorrectAnswer(targetQuestion.correctOption || "");
    } else if (targetQuestion.questionType === "code") {
      setTimeLimit(targetQuestion.timeLimit);
      setMemoryLimit(targetQuestion.memoryLimit);
      setStarterCodes((targetQuestion.starterCode as Record<string, string>) || {});
      
      const tcs = (targetQuestion.testCases as any[]) || [];
      setTestCases(tcs.map(t => ({
        input: t.input,
  ...
  if (error || !targetQuestion) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center p-6 text-center">
        <HugeiconsIcon icon={AlertCircleIcon} className="size-12 text-destructive" />
        <h2 className="text-xl font-bold mt-4">Question Not Found</h2>
        <Button onClick={() => router.push(`/dashboard/contests/${contestId}`)} className="mt-4 rounded-xl">
          Back to Contest
        </Button>
      </div>
    );
      ...
      <main className="flex-1 p-6 lg:p-10 max-w-4xl mx-auto w-full">
        <div className="rounded-3xl border border-border/40 bg-card p-6 lg:p-8 shadow-sm backdrop-blur-md space-y-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Modify the settings and test cases for this question.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-1">
              <Label className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Question Type</Label>
              <div className="text-base font-bold text-foreground capitalize bg-muted/30 px-4 py-2.5 rounded-2xl border border-border/30 w-fit">
                {questionType === "mcq" ? "Multiple Choice Question" : questionType}
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3">
              ...
              >
                {updateQuestionMutation.isPending ? <Spinner className="size-4" /> : (
                  <>
